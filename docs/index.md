...
- `src/recsys/pipeline/experiment.py` 中的单实验主干（含训练型路径）
- `src/recsys/pipeline/benchmark.py` 与 `src/recsys/pipeline/reporter.py` 中的批量调度、恢复策略与聚合报告
- `src/recsys/models/classical/` 中的 `itemcf`（非训练排序基线）+ `dssm`（双塔神经网络，首个可训练模型）
- `scripts/run_single.py`、`scripts/run_benchmark.py` 中的 CLI 实验入口

当前仍需明确保留的边界包括：

- 除 `itemcf` 和 `dssm` 外，大部分模型家族文件仍是目录预留或占位实现
