...
- `src/recsys/data` 中的数据集适配器（TAAC 2025/2026、MovieLens-1M、合成数据）、离线预处理、特征工程、负采样与 EDA 报告
...
- `src/recsys/pipeline/experiment.py` 中的单实验主干（含训练型路径，已通过 HyFormer 验证）
- `src/recsys/pipeline/benchmark.py` 与 `src/recsys/pipeline/reporter.py` 中的批量调度、恢复策略与聚合报告
- `src/recsys/models/classical/` 中的 `itemcf`（非训练排序基线）
- `src/recsys/models/unified/` 中的 `hyformer`（双塔 pointwise，已通过训练型路径验证）
- `scripts/run_single.py`、`scripts/run_benchmark.py` 中的 CLI 实验入口

当前仍需明确保留的边界包括：

- 除 `itemcf` 和 `hyformer` 外，大部分模型家族文件仍是目录预留或占位实现
- `scripts/run_ablation.py` 等辅助脚本仍待完善

因此，RecBench 当前不是"全模型已完工的 Benchmark 套件"，而是"核心运行时已成形、可从 `itemcf` 和 `hyformer` 最小闭环继续向外扩展"的推荐系统工程框架。
