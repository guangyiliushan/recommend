...
- `datasets/`：TAAC 2025、TAAC 2026、MovieLens-1M 适配器（含 dense ID remap、temporal/random split、子集加载）
- `dataset_registry.py`：数据集注册与能力发现
- `split_utils.py`：公共序列切分与用户-物品映射工具
- `preprocessor.py`：离线预处理流水线主干（chunked read、格式转换、缓存/checkpoint/数据库导出）
- `feature_engineering.py`：chunk-aware 特征工程（frequency/target/category encode、特征交叉、归一化）
- `negative_sampling.py`：大规模负采样（uniform/popularity/in-batch/mixed，item pool 缓存复用）
- `eda/`：自动化探索性数据分析，生成 ECharts 可视化 Markdown 报告
...
这一层已经完成了可训练模型的训练基础设施，已通过 `_execute_trainable_path()` 接入 experiment 主流程并经 HyFormer 验证。
...
- `experiment.py`：单实验主干、状态管理、artifact 落盘（含训练型路径）
...
1. 加载已注册的数据集 adapter（TAAC 2026 / MovieLens-1M / TAAC 2025 / 合成数据）
2. 通过模型注册表获取 `itemcf`（非训练）或 `hyformer`（训练）
3. 走 `run_experiment()` 的非训练式路径或训练式路径
4. 用 `evaluate()` 计算指标
5. 落盘标准 artifact（含 `checkpoints/`）
...
- `scripts/run_ablation.py` 等辅助脚本仍待完善
- TAAC 2026 数据集已完成 dense ID remap，确保原始稀疏 ID 压缩为连续索引，避免 embedding 越界
- 新增的离线数据管线（preprocessor、feature_engineering、negative_sampling）支持 chunk-aware 批处理，避免 OOM
