...
这一层已经完成了可训练模型的训练基础设施，但 experiment 主流程尚未把训练型模型真正接入这一层。
...
4. 用 `evaluate()` 计算 ranking 指标
5. 落盘标准 artifact
6. 用 `run_benchmark()` 和 `Reporter` 聚合多次 run

这个最小闭环的意义是验证：

- 非训练模型可以不经过 Lightning 仍接入统一主干
- 评估层能直接消费标准化预测产物
...
- `run_experiment()` 中的训练型模型路径尚未完成
- `training` 基础设施已实现，但还没有接入 experiment 主路
- 模型家族目录已齐全，但实际可运行模型仍很少
- `scripts/` 中 CLI 入口仍是骨架，不是稳定公共接口
