...
def generate_markdown_report(
    ...
    if overview.skipped:
        ...
        lines.append(f"- **反馈类型**：{overview.feedback_type}")
        lines.append(f"- **序列类型**：{overview.sequence_type}")
        lines.append(f"- **模态**：{overview.modality}\n")
    ...
    if user_item.skipped:
        ...
        if user_item.user_activity_histogram and has_chart("user_activity_histogram"):
            lines.append(_echarts_div("user_activity_histogram", "用户活跃度分布", assets_dir))
        if user_item.item_popularity_histogram and has_chart("item_popularity_histogram"):
            lines.append(_echarts_div("item_popularity_histogram", "物品流行度分布", assets_dir))
    ...
    # ---- 13. Sparsity & Cold-start Analysis ----
    lines.append("## 13. 稀疏度与冷启动分析\n")
    if sparsity.skipped:
        lines.append(_format_skipped("稀疏度分析", sparsity.skip_reason))
    else:
        if has_chart("sparsity_gauge"):
            lines.append(_echarts_div("sparsity_gauge", "交互密度", assets_dir))
        lines.append(f"- **矩阵稀疏度**：{sparsity.matrix_sparsity:.4%}")
        density_pct = (1.0 - sparsity.matrix_sparsity) * 100
        lines.append(f"- **交互密度**：{density_pct:.2f}%")
        if sparsity.item_popularity_stats:
            ips = sparsity.item_popularity_stats
            lines.append(f"- **物品**：{ips.get('total_items', 0):,.0f} 个，平均曝光 {ips.get('mean', 0):.1f} 次")
        lines.append(f"- **物品流行度 Gini 系数**：{sparsity.item_gini:.4f}（0=均匀，1=极度集中）")
        lines.append(f"- **冷启动用户占比**：{sparsity.cold_start_user_ratio:.1%}（交互数 < P5 阈值）")
        lines.append(f"- **冷启动物品占比**：{sparsity.cold_start_item_ratio:.1%}（交互数 < P5 阈值）")
        if sparsity.user_concentration:
            lines.append("- **用户交互集中度**：")
            for k, v in sparsity.user_concentration.items():
                lines.append(f"  - {k}：{v:.1%}")
        lines.append(f"- **长尾覆盖**：{sparsity.long_tail_coverage:.1%}（后 50% 物品贡献的交互占比）")
        if has_chart("item_lorenz_curve"):
            lines.append(_echarts_div("item_lorenz_curve", "物品流行度洛伦兹曲线", assets_dir))
        lines.append("")

    # ---- 14. Temporal Analysis ----
    lines.append("## 14. 时序行为分析\n")
    if temporal.skipped:
        lines.append(_format_skipped("时序分析", temporal.skip_reason))
    else:
        ts = temporal.time_span
        lines.append(f"- **时间跨度**：{ts.get('min_ts', 'N/A')} ～ {ts.get('max_ts', 'N/A')}（{ts.get('span_days', 0):,} 天）")
        lines.append(f"- **日均交互**：{temporal.daily_avg_interactions:.1f}")
        lines.append(f"- **峰值日交互**：{temporal.peak_day_interactions:,}")
        if temporal.interaction_gap_stats:
            gs = temporal.interaction_gap_stats
            lines.append(f"- **交互间隔**：均值 {gs.get('mean_hours', 0):.1f}h，中位数 {gs.get('p50_hours', 0):.1f}h，P95 {gs.get('p95_hours', 0):.1f}h")
        if has_chart("monthly_volume"):
            lines.append(_echarts_div("monthly_volume", "月度交互量趋势", assets_dir))
        if has_chart("retention_curve"):
            lines.append(_echarts_div("retention_curve", "用户留存曲线", assets_dir))
        lines.append("")

    # ---- 15. Rating Analysis ----
    lines.append("## 15. 评分分析\n")
    if rating.skipped:
        lines.append(_format_skipped("评分分析", rating.skip_reason))
    else:
        lines.append(f"- **全局平均评分**：{rating.global_mean:.2f}")
        if rating.rating_distribution:
            top_rating = max(rating.rating_distribution, key=rating.rating_distribution.get)
            lines.append(f"- **最常见评分**：{top_rating}（{rating.rating_distribution[top_rating]:.1%}）")
        if rating.rating_density:
            rd = rating.rating_density
            lines.append(f"- **评分稠密度**：P50={rd.get('p50', 0):.0f}，P95={rd.get('p95', 0):.0f}")
        if rating.user_bias_top:
            lines.append("- **最慷慨用户 Top-3**：")
            for uid, bias in rating.user_bias_top[:3]:
                lines.append(f"  - User {uid}：+{bias:.2f}")
        if rating.user_bias_bottom:
            lines.append("- **最挑剔用户 Top-3**：")
            for uid, bias in rating.user_bias_bottom[:3]:
                lines.append(f"  - User {uid}：{bias:.2f}")
        if has_chart("rating_distribution"):
            lines.append(_echarts_div("rating_distribution", "评分值分布", assets_dir))
        if has_chart("user_rating_bias"):
            lines.append(_echarts_div("user_rating_bias", "用户评分偏置", assets_dir))
        lines.append("")
